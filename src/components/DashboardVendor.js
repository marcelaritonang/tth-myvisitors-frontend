...
const DashboardVendor = () => {
  ...
  const [labs, setLabs] = useState([]); // Updated state to store roomId, name, and accessDocument
...
useEffect(() => {
  const fetchLabs = async () => {
    try {
      ...
      const accessResponse = await fetch('http://localhost:5000/access/', {
        headers: {
      ...
      const roomIds = accessData.data.map((access) => ({
        roomId: access.roomId,
        accessDocument: access.accessDocument,
      }));

      const labDetails = [];
      for (const room of roomIds) {
        const roomResponse = await fetch(`http://localhost:5000/room/${room.roomId}`);
        if (!roomResponse.ok) {
          throw new Error(`Failed to fetch room data for roomId: ${room.roomId}`);
        }
        ...
        labDetails.push({
          ...
          accessDocument: room.accessDocument,
        });
      }
  ...
  const handleSubmit = async (e) => {
    ...
    const labDetails = labs.find((lab) => lab.name === selectedLab);
    if (!labDetails) {
      setErrorMessage('Lab yang dipilih tidak valid.');
      return;
    }
  
    const token = localStorage.getItem('token');
    const visitData = {
      roomId: labDetails.roomId,
      pic,
      accessDocument: labDetails.accessDocument,
    };
    ...
    try {
      ...
      setDeviceName('');
      setErrorMessage('');
      alert('Check-in berhasil dan data telah disimpan!');
    } catch (error) {
